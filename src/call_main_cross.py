...
if __name__ == "__main__":
    ...
    for idx, row in best_params.iterrows():
        ...
        args = ["--use_pos", str(row['use_pos']), "--use_context", str(row['use_context']), 
                ...
                "--use_weights", str(row["use_weights"]), "--need_to_train_trans", "True", "--n_splits", "10"]


        # Executando script1.py com os argumentos específicos
        try:
            subprocess.run([python_interpreter, r"src/main_cross.py"] + args, capture_output=True, text=True, check=True)
